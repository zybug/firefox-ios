...
class ExtensionPanel: UIViewController, HomePanel {
    ...
    private lazy var dataSource: ExtensionDataSource? = {
        if let itemType = self.viewOptions?.itemType {
            return ExtensionDataSource(itemType: itemType, storage: Storage())
        } else {
            return nil
        }
    }()
...
private class ExtensionDataSource: NSObject {
    ...
    init(itemType: PanelItemType, storage: Storage) {
...
private class Storage: NSObject {
    var items: [HomeStorageItem] = []
}

extension Storage: HomeStorage {
